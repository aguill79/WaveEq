...
int main(int argc, char *argv[]) {
   ...
   // This is the part that will need to be made parallel
   rho2=rho*rho;
    ...
    #pragma omp parallel
   {
        ...
        // A bit of code to help me track what threads are doing what work
        int thread_num = omp_get_thread_num();
        int thread_num_i = thread_num*active_points+thread_num;
        if(check_threads){

            checkpoints[thread_num_i] = thread_num*100;
            //printf("[%d] %d\n", thread_num_i, checkpoints[thread_num_i]);
       ...
       for (t=0; t<steps; t++) {
        ...
        if(check_threads){
            checkpoints[thread_num_i+offset] = t;
            //printf("%d: %d\n", thread_num_i+offset, checkpoints[thread_num_i+offset]);
            offset++;
        }
   ...
   printf("Processing time : %f (ms)\n",timer_val*1000.);
    ...
    if(check_threads){
        ...
        int temp;
        printf("Elements of thread numbers\n");
        for(int i=0; i<thread_count; i++){
            temp = i*active_points+i;
            
            printf("Thread element: %d\n", temp);
            printf("recorded thread num: %d\n", checkpoints[temp]);
        }

        printf("Thread iteration distribution\n");
        printf("(thread numbers are multiplied by 100)\n");
